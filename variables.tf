...
variable "budget_limit_amount" {
  description = "Monthly budget limit amount in EUR"
  type        = number
  default     = 5
  
  validation {
    condition = var.budget_limit_amount > 0 && var.budget_limit_amount <= 10
    error_message = "Budget limit must be between 1 and 10 EUR."
  }
}
